...
- [计算机网络](#计算机网络)
  - [浏览器输入网址后发生了什么?](#浏览器输入网址后发生了什么)
  - [三次握手](#三次握手)
    - [三次握手的流程是什么样的?](#三次握手的流程是什么样的)
    - [为什么需要三次握手?](#为什么需要三次握手)
  - [四次挥手](#四次挥手)
    - [四次挥手的流程是什么样的?](#四次挥手的流程是什么样的)
    - [为什么需要四次挥手?](#为什么需要四次挥手)
    - [为什么 TIME\_WAIT 状态需要等待 2MSL?](#为什么-time_wait-状态需要等待-2msl)
    - [为什么需要 TIME\_WAIT 状态?](#为什么需要-time_wait-状态)
    - [TIME\_WAIT 过多有什么危害?](#time_wait-过多有什么危害)
  - [TCP](#tcp)
    - [重传机制有哪些?分别是什么样的?](#重传机制有哪些分别是什么样的)
    - [滑动窗口是什么?有什么作用?](#滑动窗口是什么有什么作用)
    - [流量控制是什么?有什么作用?](#流量控制是什么有什么作用)
...
是一种机制，可以让「发送方」根据「接收方」的`实际接收能力`控制发送的`数据量`，这就是所谓的流量控制

发送方发送数据的时候，要考虑接收方的处理能力，如果一直疯狂发送数据，接收方处理不过来，就会导致触发重发机制，从而导致网络流量浪费。

是通过滑动窗口实现的, 通过 `Window` 字段的大小来控制

**窗口关闭**

如果窗口大小为 `0`, 那么发送方就 `不能发送数据了`, 这个时候就叫做 `窗口关闭`

处理完成后，接收方会发送一个 `Window` 大小不为 `0` 的 `ACK` 报文, 这样发送方就可以继续发送数据了

但如果这个非 0 的 ACK 报文丢失的话，就会进入类似死锁的状态
